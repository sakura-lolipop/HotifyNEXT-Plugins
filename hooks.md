...
> 🌐（给 AI：与用户的交互语言一律跟随用户——本规则以中文书写，但对话、产出的说明与步骤
> 应使用用户自己的语言。）
...
**部署说明第一行必须是「你需要提供」清单**（2026-08-29 定）：显式列出每个**只有用户才知道的值**
（密钥/域名/设备 slug…），用 `<尖括号占位符>` 标注 + 一句"从哪来"。执行安装的 AI（不一定是作者）
以占位符为问询清单——见到占位符且拿不到值，**必须停下问用户，禁止编造或跳过**。顺序铁律：
密钥类占位符必须**先拿到再放文件**（无 secret 重启 = server 整体 fatal，拖挂生产服务）。例：

```yaml
# ── 你需要提供 ──
#  HOOKS_MEMOS_SECRET = <memos 建 webhook 时显示的 whsec_ 开头整串>（先拿这个再装）
#  <你的域名>         = 你的 Hotify 公网地址（源端 webhook URL 用）
```

**问询形态（UX 纪律，2026-08-29）**：问用户时按值类型选形态——**可枚举的问题**（部署形态/
源端在内网吗/定向还是广播/选哪个插件）**优先用 agent 的结构化提问能力**（如 Claude Code 的
AskUserQuestion / 各 agent 的 questions 类工具：选项+说明+一次批量多问），**禁止退化成自由
文本连环追问**（"请输入域名：""请输入密钥："一问一答是最差 UX）；**凭证类长串**（whsec_/token，
无法选项化）才用粘贴式问答，且一次说清"要什么+从哪抄"，不让用户猜你要什么格式。
...
3. **源端配置**（按凭证分岔）：
   ...
   - 无凭证 → 停在这步，把"点哪里/抄哪个值"压成 ≤2 条可照抄指令给用户，完成后继续
     （泛化：部署说明里**任何 `<占位符>`** 拿不到值都同理——停下问用户，禁止编造/跳过/先放文件赌一把）
4. **验证**：触发一个真事件（源端 Test 按钮/造一条真数据），确认消息落桶（`[hooks] … delivered`）
   或手机收到通知。**全绿才算完成**——装上但没验过 = 未完成。

**排障模式**（2026-08-29 二次升级——用户回来说"装完没反应/帮我检查"时）。**手上没有本文件？**
（新对话/新工具）——让用户把出问题的插件 yaml 发来：**每个插件文件头的「排障」段就是本模式
的现场版**（判据行/常见错分流都有），照它执行即可。有本文件时按此序——此时 server 起得来
（格式类 fatal 已被排除），**别先翻 yaml**——值对不对在文件里看不出来（格式合法但值错=最常见
的静默失败型：server 全绿、源端全 401，唯一痕迹在日志）。按先验排序：

1. **日志先行**：`docker logs` / `hotify.log` 里 grep `[hooks]`——
   `401` = secret 两边不一致（值错，命中率最高）｜`filter miss` = 源发的是别的事件（正常）｜
   `delivered` 但用户没收到 = 设备侧（0 设备 onboard？）｜**一行都没有** = 源端没发（SSRF 私网
   参数？）或根本没装上（404 探针分）
2. **主动问用户三问**（按命中率序）：secret 是 `<占位符>` 原样整串照抄的吗（含前缀）？
   源端 webhook 建好且事件/触发器选对了吗？源端在内网吗（SSRF 放行参数配了吗——部署说明里的
   ⚠️ 行）？
3. **yaml 最后**：能走到"装完"阶段说明文件已过三关（启动自检绿），文件本身几乎不可能是问题。
